...
extern vu8 dirmode;		//横竖屏模式:0,竖屏模式;1,横屏模式.
	...
	if(lcdltdc.width!=0)//RGB屏
	{
		...
		}else//横屏
		{
			for (col = cinfo->output_width >> 1; col > 0; col--) 
			{
				...
				//outptr[RGB_BLUE] =  range_limit[y + cblue];
				//outptr += RGB_PIXELSIZE; 
				outlinebuf[cinfo->output_width-(col<<1)]=(range_limit[y + cblue] >> 3)<<11 | (range_limit[y + cgreen] >> 2) << 5 | range_limit[y + cred] >> 3;
				...
				//outptr[RGB_BLUE] =  range_limit[y + cblue];
				//outptr += RGB_PIXELSIZE;
				outlinebuf[cinfo->output_width-(col<<1)+1] = (range_limit[y + cblue] >> 3)<<11 | (range_limit[y + cgreen] >> 2) << 5 | range_limit[y + cred] >> 3;
	...
	if(lcdltdc.width!=0)//RGB屏
	{
		...
		}else//横屏
		{
			...
			for (col = cinfo->output_width >> 1; col > 0; col--) 
			{
				...
				/* Fetch 4 Y values and emit 4 pixels */
				y  = GETJSAMPLE(*inptr00++);
				outlinebuf[cinfo->output_width-(col<<1)] = range_limit[y + cblue] >> 3 | (range_limit[y + cgreen] >> 2) << 5 | (range_limit[y + cred] >> 3) << 11;
	 
				y  = GETJSAMPLE(*inptr00++);
				//pixel.color.B = range_limit[y + cblue] >> 3;
				//pixel.color.G = range_limit[y + cgreen] >> 2;
				//pixel.color.R = range_limit[y + cred] >> 3;
				outlinebuf[cinfo->output_width-(col<<1)+1] = range_limit[y + cblue] >> 3 | (range_limit[y + cgreen] >> 2) << 5 | (range_limit[y + cred] >> 3) << 11;
			}
		}
		LTDC_Color_Fill2(imgoffx,outlinecnt,cinfo->output_width+imgoffx-1,outlinecnt,outlinebuf);//DM2D填充 
		outlinecnt++; 
