...
extern u16 sms_yoff;			//显示在y轴方向的偏移量
extern u16 sms_xoff;			//显示在x轴方向的偏移量(实际显示宽度=256-2*sms_xoff)
...
void render_line(int line)
{
	...
	u32 temp;
	u32 color=0;
	u32 *plcd=ltdc_framebuf[lcdltdc.activelayer];	//指向显存地址减少数组访问,提高速度
	if(line>=192)return;  
	...
	{ 	
		if(lcddev.id==0X4342)
		{
			...
			{  
				...
				*(u16*)((u32)plcd+offset)=SMS_VDP->SMS_Palette[SMS_VDP->linebuf[i]&0x1F];//得到颜色值
			} 
		}else
		{
