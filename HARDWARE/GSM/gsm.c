...
//命令处理完时调用,与gsm_send_cmd成对使用/多个与gsm_send_cmd后调用.
...
{
	u8 *p1; 
	if(gsmdev.cmdon==0&&gsmdev.mode==0&&USART3_RX_STA==0)//非指令等待状态.拨号/短信模式/且没有收到任何数据,才允许查询
	{
		...
		{ 
			...
			{ 
				...
				if(p1)
				{
					...
					//sms部分通用设置
					gsm_send_cmd("AT+CMGF=1","OK",100);			//设置文本模式 
					gsm_send_cmd("AT+CSCS=\"UCS2\"","OK",100);	//设置TE字符集为UCS2 
					gsm_send_cmd("AT+CSMP=17,0,2,25","OK",100);	//设置短消息文本模式参数 
				}
