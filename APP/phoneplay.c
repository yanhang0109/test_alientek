...
{ 
	...
	if(bkcolor&&pdis->phonebuf&&f_phone)//内存申请成功了
	{
		...
		{ 
			...
			if(res==FR_OK)
			{
				asc2_2814=(u8*)gui_memex_malloc(f_phone->obj.objsize);	//为大字体开辟缓存地址
		...
		}else if(lcddev.width==320&&asc2_3618==0)
		{
			...
			if(res==FR_OK)
			{
				asc2_3618=(u8*)gui_memex_malloc(f_phone->obj.objsize);	//为大字体开辟缓存地址
		...
		}else if(lcddev.width>=480&&asc2_5427==0)
		{
			...
			if(res==FR_OK)
			{
				asc2_5427=(u8*)gui_memex_malloc(f_phone->obj.objsize);	//为大字体开辟缓存地址
		...
		phone_show_calling(pdis,&gsmdev);//来电中
				...
				if(res&&((p_btn[i]->sta&(1<<7))==0)&&(p_btn[i]->sta&(1<<6)))//有按键按下且松开,并且TP松开了
				{
					if(i<12)//1~#
					{
						if(gsmdev.mode==2)//仅在通话中有效
						{
							...
							phone_show_input(pdis,&gsmdev);//显示输入数据 
						}
					}
					if(i==12)//接听来电
					{
