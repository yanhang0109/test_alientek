...
{
{"ALIENTEK 阿波罗","ALIENTEK 阿波羅","ALIENTEK Apollo",},
...
void sysset_system_update(u8*caption,u16 sx,u16 sy)
{
	...
	res=app_system_file_check("0");			//先检查SD卡
  	if(res)res=app_system_file_check("3");	//SD卡有问题，再检查U盘
	if(res)
	{ 
  	...
  	if(twin!=NULL)
	{
		...
		if(res!=0XFF)//更新有误
		{
			res=app_system_update(sysset_system_update_cpymsg,"3:");	//尝试U盘更新	
		}
...
void sysset_system_info(u16 x,u16 y,u8*caption)
{
  	...
  	if(twin!=NULL&&rbtn!=NULL)
	{
		...
		for(i=0;i<11;i++)
		{
	 		...
	 		if(i==2||i==3||i==4||i==5)//SD卡/FLASH盘  容量
			{
				dtsize=0;
				dfsize=0;
				if(i==2||i==4||i==5)
				{
					...
					else if(i==4)exf_getfree("2:",&dtsize,&dfsize);	//得到NAND FLASH剩余容量和总容量 
					else exf_getfree("3:",&dtsize,&dfsize);			//得到U盘剩余容量和总容量 
					dtsize>>=10;
					dfsize>>=10;
...
void sysset_system_about(u16 x,u16 y,u8*caption)
{
  	...
  	if(twin!=NULL&&rbtn!=NULL)
	{
		...
		strcat((char*)tbuf,(const char*)verbuf);
 		...
 		gui_show_string("Copyright (C) 2016~2026",x+10,y+32+34+20*1,180,12,12,BLACK);//显示信息
