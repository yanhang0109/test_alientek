...
void sms_send_sms(sms_node* pnode,u8 mode)
{
	...
	{
		pnum_height=80;pnum_fsize=32;
		rmsg_height=40;rmsg_fsize=16;
		rmemo_height=512;
		smemo_height=192;smemo_width=450;smemo_offx=30;sarea_height=220;
		sbtn_width=150;sbtn_height=100;sbtn_fize=32;
		smsg_height=40;smsg_fsize=24;	
		t9height=428;
	} 
		...
		{ 
			if(strlen((char*)tedit->text)>0)
			{
				...
				{ 		 	
					...
					u3_printf("%s",str);	 
					gsm_send_cmd((u8*)0X1A,"+CMGS:",0);		//发送结束符,开启发送,不等待
					calendar_get_time(&calendar);
