...
u8 calc_play(void)
{
	...
	else
	{
		if(lcddev.width<=272)
		{
			...
			if(res==FR_OK)
			{
				asc2_2814=(u8*)gui_memex_malloc(f_calc->obj.objsize);	//为大字体开辟缓存地址
		...
		}else if(lcddev.width==320)
		{
			...
			if(res==FR_OK)
			{
				asc2_3618=(u8*)gui_memex_malloc(f_calc->obj.objsize);	//为大字体开辟缓存地址
		...
		}else if(lcddev.width>=480)
		{
			...
			if(res==FR_OK)
			{
				asc2_5427=(u8*)gui_memex_malloc(f_calc->obj.objsize);	//为大字体开辟缓存地址
